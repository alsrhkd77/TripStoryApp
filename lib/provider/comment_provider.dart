...
class CommentProvider {
  Future<List<Comment>> fetchAllComment(int feedId) async {
    http.Response _response =
        await http.get(AddressBook.comment + feedId.toString());
    ...
    if (state == 'success') {
      ...
      throw Exception(
          'Failed to load comment list value\n${resData['errors']}');
    } else {
  ...
  Future<void> removeComment(int commentId) async {
    http.Response _response =
        await http.delete(AddressBook.comment + commentId.toString());
    var resData = jsonDecode(_response.body);
    if (resData['result'] == 'success') {
      return 'success';
    } else if (resData['errors'] != null) {
