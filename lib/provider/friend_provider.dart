...
class FriendProvider {
  Future<List<Friend>> fetchAllFollower(String nickName) async {
    ...
    http.Response _response =
        await http.get(AddressBook.getFollowers + nickName);
    ...
    if (state == 'success') {
      ...
      throw Exception(
          'Failed to load follower list value\n${resData['errors']}');
    } else {
  ...
  Future<List<Friend>> fetchAllFollowing(String nickName) async {
    List result = new List<Friend>();
    http.Response _response =
        await http.get(AddressBook.getFollowing + nickName);
    var resData = jsonDecode(_response.body);
    var state = resData['result'];
    if (state == 'success') {
      for (var data in resData['followerInfos']) {
      ...
      throw Exception(
          'Failed to load following list value\n${resData['errors']}');
    } else {
  ...
  Future<Friend> fetchProfile(String nickName) async {
    http.Response _response = await http
        .get(AddressBook.getFriendProfile + nickName + '/' + Owner().id);
    var resData = jsonDecode(_response.body);
    var state = resData['result'];
    if (state == 'success') {
  ...
  Future<void> unfollow(String nickName) async {
    http.Response _response = await http
        .delete(AddressBook.follow + '/' + Owner().id + '/' + nickName);
    var resData = jsonDecode(_response.body);
    var state = resData['result'];
    if (state == 'success') {
      return 'success';
